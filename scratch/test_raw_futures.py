...
def main():
    ...
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            non_zero = [b for b in data if float(b.get("balance", 0.0)) > 0 or float(b.get("crossWalletBalance", 0.0)) > 0]
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            non_zero_assets = [a for a in assets if float(a.get("walletBalance", 0.0)) > 0 or float(a.get("marginBalance", 0.0)) > 0]
            ...
            print("Total Wallet Balance:", data.get("totalWalletBalance"))
            print("Total Margin Balance:", data.get("totalMarginBalance"))
        else:
            print("Status:", res.status_code, "Body:", res.text)
    except Exception as e:
        print("Exception:", e)
